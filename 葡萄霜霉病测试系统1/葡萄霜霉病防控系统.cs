...
namespace 葡萄霜霉病防控测试系统1
{
	public partial class 葡萄霜霉病防控系统 : Form
	{
			...
			while (true)
			{
				try
				{
					...
					else if (s.Substring(0, 1) == "f")
					{
						...
						//Hashtable ht = JsonConvert.DeserializeObject<Hashtable>(s.Substring(1));
						//Hashtable ht = JsonConvert.DeserializeObject<Hashtable>(s.Substring(s.IndexOf('{'), s.IndexOf("}") - s.IndexOf("{") + 1));
						//if (ht.Contains("fileName") && ht.Contains("fileType"))
						//{
						//获取文件名的长度
						byte[] btFileNameLength = new byte[4];
						Buffer.BlockCopy(buffer, 1, btFileNameLength, 0, 4);
						//转成int
						int fileNameLength = BitConverter.ToInt32(btFileNameLength, 0);
						//如果文件名长度不为0
						if (fileNameLength != 0)
						{
							//为接收文件开一个新的线程
							Thread td = new Thread(() => {
								ReceiveFile(buffer, s, r, fileNameLength);
							});
		...
		private bool ReceiveFile(byte[] buffer, string strReceive, int length, int fileNameLength)
		{
			string fileNameSuffix = strReceive.Substring(strReceive.LastIndexOf("."));
			string fileName = strReceive.Substring(strReceive.LastIndexOf("\\") + 1);
			//将保存对话框的属性赋对应的值
			SaveFileDialog sfd = new SaveFileDialog();
			...
			if (sfd.ShowDialog() == DialogResult.OK)
			{
				...
				using (FileStream fs = new FileStream(savePath, FileMode.Create, FileAccess.Write))
				{
					//buffer 的内容
					//"f" + 文件名长度 + 文件 + 文件名
					fs.Write(buffer, 5, length - 5 - fileNameLength);
					fs.Flush();
					fs.Close();
				}
				//获取文件名和路径
